...
def test_default_dashboard_password_is_rejected_when_router_mounted(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    try:
        response = configured_client.post(
            "/login",
            data={"password": auth.DEFAULT_DASHBOARD_PASSWORD},
            follow_redirects=False,
        )
        assert response.status_code == 401
        assert not configured_client.cookies.get(auth.SESSION_COOKIE_NAME)
    finally:
        configured_client.close()
        auth.reset_state()


def test_configured_default_dashboard_password_value_is_rejected(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
