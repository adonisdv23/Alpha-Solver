...
p.add_argument("--track",default=""); p.add_argument("--accept",required=True)
p.add_argument("--targets",required=True); p.add_argument("--out",required=True)
a=p.parse_args()
spec = f"""# CODE SPEC — {a.id} · {a.title} ({a.track})
...
with open(a.out,"w") as f: f.write(textwrap.dedent(spec))
print("Wrote", a.out)
