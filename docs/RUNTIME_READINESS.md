...
| FastAPI service `/v1/solve` | Verified local/offline / Partial | Existing route calls local deterministic ToT/ReAct-style runtime paths by default and is covered by service/API tests. When `MODEL_PROVIDER=openai` is explicitly selected, the route can execute through the OpenAI provider client foundation and emits minimal no-secret provider lifecycle telemetry for started, completed, failed, and timeout outcomes. | Provider budget accounting, replay integration, Prometheus/Grafana/OTel provider instrumentation, and full SAFE-OUT/fallback orchestration remain follow-up work. | None for local API tests; live OpenAI use requires `OPENAI_API_KEY`. Default CI uses fake/mocked provider tests and makes no live calls. | Service tests and `.specs/PROVIDER-OPENAI-001.md`. |
...
| OpenAI live/production-hardened execution | Partial / future hardening | The OpenAI provider client foundation exists; FastAPI `/v1/solve` has explicit opt-in OpenAI provider integration via `MODEL_PROVIDER=openai`; minimal no-secret lifecycle telemetry exists for that service path; provider failures are normalized safely at the current service integration level; default CI uses fake/mocked tests and makes no live provider calls. | Env validation does not prove live provider usability; provider budget/cost accounting, replay integration, Prometheus/Grafana/OTel provider instrumentation, deeper SAFE-OUT/fallback orchestration, optional gated live OpenAI smoke tests, and production hardening remain future work. | Live OpenAI use requires `OPENAI_API_KEY`, account/model access, billing/quota, and network. Default local/offline use and default CI require no provider credentials. | `.specs/PROVIDER-OPENAI-001.md`, `alpha/providers/`, service API tests, and future provider-readiness specs. |
