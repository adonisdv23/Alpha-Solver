...
| FastAPI service `/v1/solve` | Verified local/offline / Partial | Existing route calls local deterministic ToT/ReAct-style runtime paths by default and is covered by service/API tests. When `MODEL_PROVIDER=openai` is explicitly selected, the route can execute through the OpenAI provider client foundation, emits minimal no-secret provider lifecycle telemetry for started, completed, failed, and timeout outcomes, and records account-only post-call provider cost accounting after successful OpenAI provider results. | Hard/soft budget enforcement, preflight blocking, persistent budgets, billing integration, SAFE-OUT budget behavior, replay integration, Prometheus/Grafana/OTel provider instrumentation, dashboards/tracing, CLI budgeting, portable solver changes, and full SAFE-OUT/fallback orchestration remain follow-up work. | None for local API tests; live OpenAI use requires `OPENAI_API_KEY`. Default CI uses fake/mocked provider tests and makes no live calls. | Service tests and `.specs/PROVIDER-OPENAI-001.md`. |
...
1. The minimal OpenAI provider client foundation exists; `/v1/solve` has explicit opt-in OpenAI provider integration and account-only post-call cost accounting for successful OpenAI provider results, but enforcement, persistence, live smoke tests, dashboards/tracing, CLI budgeting, portable solver changes, and broader production hardening remain follow-up work.
