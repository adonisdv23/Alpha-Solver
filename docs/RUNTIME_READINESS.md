...
| OpenAI live/production-hardened execution | Partial / future hardening | The OpenAI provider client foundation exists; FastAPI `/v1/solve` has explicit opt-in OpenAI provider integration via `MODEL_PROVIDER=openai`; provider failures are normalized safely at the current service integration level; default CI uses fake/mocked tests and makes no live provider calls. | Env validation does not prove live provider usability; provider telemetry and provider budget/cost accounting are not fully integrated; deeper SAFE-OUT/fallback orchestration, optional gated live OpenAI smoke tests, and production hardening remain future work. | Live OpenAI use requires `OPENAI_API_KEY`, account/model access, billing/quota, and network. Default local/offline use and default CI require no provider credentials. | `.specs/PROVIDER-OPENAI-001.md`, `alpha/providers/`, service API tests, and future provider-readiness specs. |
