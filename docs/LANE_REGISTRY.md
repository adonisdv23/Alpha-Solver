...
| Lane | State | Evidence |
|------|-------|----------|
| Routed-vs-plain manual pilot outputs | **current control posture** | `ALPHA-SOLVER-ROUTED-VS-PLAIN-PILOT-OUTPUTS-001` records manual no-provider prompt-contract simulation output pairs and route metadata. Selected next state is `OPERATOR_REVIEW_REQUIRED_AFTER_ROUTED_VS_PLAIN_PILOT_OUTPUTS_001`. |

## Next ready / current selected state

| State | Lifecycle | Notes |
|-------|-----------|-------|
| **`OPERATOR_REVIEW_REQUIRED_AFTER_ROUTED_VS_PLAIN_PILOT_OUTPUTS_001`** | **review-only selected next state** | Manual no-provider prompt-contract simulation outputs are recorded after `ALPHA-SOLVER-ROUTED-VS-PLAIN-PILOT-OUTPUTS-001`; no Alpha runtime, `/v1/solve`, provider/local-model/hosted-model/tool/web execution, current external research, scoring, unblinding, source-map work, Google Sheets mutation, dependency addition, dashboard/public API exposure, deployment, production/public readiness, benchmark, provider/local-model/tool-quality, security/privacy completion, autonomous-readiness, or Alpha-superiority claim is authorized. Prior selected next state was `OPERATOR_REVIEW_REQUIRED_AFTER_ROUTED_VS_PLAIN_PILOT_AUTHORIZATION_001`. |
...
OPERATOR_REVIEW_REQUIRED_AFTER_ROUTED_VS_PLAIN_PILOT_AUTHORIZATION_001 ← prior review-only selected next state
        │
        ▼
ALPHA-SOLVER-ROUTED-VS-PLAIN-PILOT-OUTPUTS-001 ← manual no-provider simulation outputs completed
        │
        ▼
...
Current selected next state after routed-vs-plain manual pilot outputs: `OPERATOR_REVIEW_REQUIRED_AFTER_ROUTED_VS_PLAIN_PILOT_OUTPUTS_001`.
...
| Evidence value | Records 12 output pairs for `RVP-001` through `RVP-012`, with one plain baseline output, one routed Alpha output, and route metadata per task. |
