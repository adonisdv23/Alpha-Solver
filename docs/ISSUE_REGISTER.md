...
| ISS-003 | P2 | CORS default risk | REMEDIATED_FOR_RR_01 | `alpha/core/config.py` (local-only default origins; rejects wildcard with credentials), `service/app.py` (`allow_credentials=cfg.cors.allow_credentials`) | DEF-002 CORS boundary fix | Keep public exposure blocked until remaining DEF-002 auth/tenancy/data/supply-chain items close | No | Residual review before public |
