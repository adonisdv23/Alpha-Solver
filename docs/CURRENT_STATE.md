...
| Field | Value |
|-------|-------|
| Last merged PR | **#509** — `docs(openai): add local token smoke retry capture packet` (merged 2026-06-13T13:52:58Z) |
| Current controlling lane | `LOCAL-OPENAI-TOKEN-SMOKE-CAPTURE-RETRY-001` (PR #509, **blocked before provider call**) |
| Next selected lane | **`OPENAI-PROJECT-BILLING-BOUNDARY-CLARIFICATION-001`** (docs/operator clarification, no provider call) |
| Next provider-call attempt (only after clarification) | `LOCAL-OPENAI-TOKEN-SMOKE-CAPTURE-RETRY-002` |
...
- **First real OpenAI token smoke** — blocked on OpenAI project/billing
  verification (PR #509). No provider call until clarified.
