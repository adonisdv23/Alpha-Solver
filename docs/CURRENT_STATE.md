...
> Source-of-truth navigation doc. Last verified **2026-06-17** for routed-vs-plain pilot output collection prep.
> This lane records blank operator-fillable templates and a runbook for future routed-vs-plain output collection. It does not record completed pilot outputs, run Alpha runtime, invoke `/v1/solve`, call providers, run hosted or local models, execute tools, browse, score outputs, unblind, mutate Sheets, deploy, or make production/public/benchmark/provider/local-model/tool/security/privacy/autonomous-readiness/Alpha-superiority claims.

## Current verified phase

**Routed-vs-plain pilot output collection prep completed: the selected next state is review-only after preparation of blank operator-fillable templates.**
...
| Field | Value |
|-------|-------|
| Latest verified completed lane in this wave | **`ALPHA-SOLVER-ROUTED-VS-PLAIN-PILOT-OUTPUT-COLLECTION-PREP-001`** |
| Source-of-truth sync | Current docs record the completed routed-vs-plain output collection preparation lane and a review-only selected next state |
| Closed-unmerged superseded PR | **#561** - superseded by merged PR #562 |
| Current controlling posture | Operator review required after routed-vs-plain pilot output collection prep |
| Selected next state | **`OPERATOR_REVIEW_REQUIRED_AFTER_ROUTED_VS_PLAIN_PILOT_OUTPUT_COLLECTION_PREP_001`** |
| Strategic boundary | This review-only state records operator-fillable output collection templates only; it does not prove runtime, `/v1/solve`, provider, hosted-model, local-model, tool, web, benchmark, production/public readiness, security/privacy, quality, autonomous-readiness, or Alpha-superiority claims and does not authorize scoring or unblinding |
