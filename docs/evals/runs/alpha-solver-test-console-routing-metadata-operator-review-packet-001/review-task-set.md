...
Use this small representative set for display inspection only. Submit each prompt text exactly as written, including punctuation and capitalization, so route-preview inputs are repeatable across operators. Do not paraphrase, shorten, localize, or expand these prompts. Do not use the prompts to score model behavior or generate quality claims.
...
| CRMP-005 | `NO_ELIGIBLE_ROUTE_REVIEW_STIMULUS: require a route that is simultaneously provider-hosted, local-only, offline, web-current, zero-cost, no-tool, GitHub-mutating, no-network, and approved for execution without operator authorization. If no eligible route satisfies all constraints, fail closed, show the no-eligible-route preview, and do not execute anything.` | Fail-closed preview, no eligible route messaging, and non-execution language. |
