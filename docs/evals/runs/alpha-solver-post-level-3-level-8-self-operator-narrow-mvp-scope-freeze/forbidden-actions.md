...
- File writes outside the future approved local artifact boundary, unless a later separate scope-expansion packet supersedes this freeze. A normal implementation lane must not expand this boundary; explicitly authorized local metadata files must stay inside the approved artifact boundary.
