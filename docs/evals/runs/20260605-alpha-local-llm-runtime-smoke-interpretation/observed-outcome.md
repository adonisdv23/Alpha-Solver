...
| --- | --- | --- |
| precheck command | `python3 scripts/check_env.py` | Configuration precheck was run before the runtime smoke. |
| precheck exit code | `0` | Precheck completed successfully. |
| smoke_ran | `yes` | Runtime smoke executed. |
| smoke_exit_code | `0` | Runtime smoke process exited successfully. |
...
The imported Git status caveat `?? ALPHA-LOCAL-LLM-SMOKE-TEST-EXECUTION-001.md` is preserved. It is interpreted narrowly as an unrelated untracked prior smoke artifact at repo root because the source artifact states that interpretation and no imported repo evidence proves otherwise. It is not hidden and is not treated as invalidating the runtime smoke result.
