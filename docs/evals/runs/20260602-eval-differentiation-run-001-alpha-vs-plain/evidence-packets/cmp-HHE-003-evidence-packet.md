# cmp-HHE-003 · Evidence Packet

## Packet identity

| Field | Value |
| --- | --- |
| `packet_id` | `cmp-HHE-003-evidence-packet` |
| `comparison_id` | `cmp-HHE-003` |
...
| `prompt_id` | `HHE-003` |
| `prompt_family` | `higher-headroom repo evidence discipline` |
...
| `paired_output_capture path` | `paired-output-captures/cmp-HHE-003-paired-output-capture.md` |
...
The official blind scorer preferred Output B for `cmp-HHE-003`. After applying the approved unblinding map, the plain total is 69, the Alpha total is 68, and the Alpha-minus-plain delta is -1. This is a local result for this one comparison only.
