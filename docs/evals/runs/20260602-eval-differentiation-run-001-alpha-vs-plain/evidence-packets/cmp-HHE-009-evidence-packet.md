# cmp-HHE-009 · Evidence Packet

## Packet identity

| Field | Value |
| --- | --- |
| `packet_id` | `cmp-HHE-009-evidence-packet` |
| `comparison_id` | `cmp-HHE-009` |
...
| `prompt_id` | `HHE-009` |
| `prompt_family` | `higher-headroom safe agent instruction cleanup` |
...
| `paired_output_capture path` | `paired-output-captures/cmp-HHE-009-paired-output-capture.md` |
...
- Plain surface after unblinding: Output B
- Alpha surface after unblinding: Output A
...
The official blind scorer preferred Output B for `cmp-HHE-009`. After applying the approved unblinding map, the plain total is 45, the Alpha total is 44, and the Alpha-minus-plain delta is -1. This is a local result for this one comparison only.
