# cmp-HHE-007 · Evidence Packet

## Packet identity

| Field | Value |
| --- | --- |
| `packet_id` | `cmp-HHE-007-evidence-packet` |
| `comparison_id` | `cmp-HHE-007` |
...
| `prompt_id` | `HHE-007` |
| `prompt_family` | `higher-headroom go-no-go memo` |
...
| `paired_output_capture path` | `paired-output-captures/cmp-HHE-007-paired-output-capture.md` |
...
The official blind scorer preferred Output A for `cmp-HHE-007`. After applying the approved unblinding map, the plain total is 68, the Alpha total is 64, and the Alpha-minus-plain delta is -4. This is a local result for this one comparison only.
