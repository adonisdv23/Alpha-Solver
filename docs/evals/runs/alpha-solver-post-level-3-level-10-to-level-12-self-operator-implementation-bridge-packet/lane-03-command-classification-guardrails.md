# Lane 03: command classification guardrails

## Objective

Tighten command classification so local preflight and dry-run lanes block unsafe command surfaces before execution.

## Source evidence to verify

Verify the preflight forbidden-command list and approval stop-state codes for provider, external API, credential, browser, deployment, billing, route exposure, fallback, hosted fallback, evidence promotion, and source-artifact mutation blocks.

## Allowed changes

Local-only command classifier helpers, classifier fixtures that are inert, deterministic classifier tests, and a lane-completion docs packet.
...
Blocked-command detection for network/package install/remote fetch/provider/browser/deployment/billing/credential patterns, ambiguous command stop, and changed-file scope proof.
...
If Lane 02 is merged and GS done, select command classification guardrails; otherwise split or fall back.
