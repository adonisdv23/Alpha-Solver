...
| 3 | Alpha triggers a mandatory stop condition through fabricated results, unsafe claims, missing-artifact reconstruction, raw-output/map boundary violation, Batch C/runtime creep, or uninterpretable output | `refine stop-condition behavior` | Do not average this away with otherwise positive ratings. |
...
| 6 | Low-headroom, brevity, answer-first, caveat-length, or over-scaffolding defects dominate, while claim/evidence/stop boundaries remain intact | `refine brevity/control again` | Use when the main defect is response shape or headroom control. |
| 7 | Next-lane tasks repeatedly produce multiple lanes, unclear operator action, or unsafe optional leaps, while other boundaries remain intact | `refine claim-boundary/evidence-boundary behavior` | Wrong next-lane selection usually risks claim or scope creep. |
