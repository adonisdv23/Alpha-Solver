# Command plan before repair

Verbatim content of `execution-command-plan.md` as merged on `main` at
`e04d4cc` (before the repair). The two defects are the `git fetch origin
main` precondition in step 1 and the quoted heredoc relying on
`Path("$ROOT")` in step 2. **This defective plan was never executed.**
