...
| PR #549 API-shape gate | `main` records that sidecars must not assume the current Alpha Solver route contract is OpenAI-compatible. | Bridge must not proceed until request mapping/API-shape compatibility is selected and tested. |
| `POST /v1/solve` | Active API route with API-key validation and rate limiting; classified unsafe to expose publicly by existing evidence. | Do not expose or reuse as a sidecar API without a separate API-shape, authn/authz, tenancy, CORS/CSRF, and cost-control decision. |
...
- Preserve tenant/cost/telemetry/audit metadata as explicit, non-public bridge fields.
