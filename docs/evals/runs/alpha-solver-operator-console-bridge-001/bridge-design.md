...
1. **Alpha Solver controlled endpoint**: the sidecar may call only an approved Alpha Solver endpoint or CLI bridge seam.
...
5. **Envelope preservation**: responses must preserve Alpha Solver response shape, SAFE-OUT behavior, request IDs, mode/route metadata, cost/accounting metadata when present, telemetry hooks, and evidence labels.
