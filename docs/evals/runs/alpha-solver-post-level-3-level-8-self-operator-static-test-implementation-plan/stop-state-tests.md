...
- `STOPPED_FALLBACK_PATH_DETECTED`
- `STOPPED_EVIDENCE_PROMOTION_DETECTED`
...
| Missing stop state for fallback configuration, local-to-provider fallback, or hosted fallback block. | `SELF_OPERATOR_STOP_STATE_REQUIRED`. |
| Missing stop state for evidence/readiness/benchmark/score promotion labels. | `SELF_OPERATOR_STOP_STATE_REQUIRED`. |
...
| Stop state text claims evidence promotion, readiness promotion, benchmark promotion, score promotion, or runtime trust. | `SELF_OPERATOR_STOP_STATE_PROMOTION_BLOCKED`. |
